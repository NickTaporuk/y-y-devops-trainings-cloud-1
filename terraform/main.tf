...
locals {
  folder_id = "b1gecf9ktsv4dv95at0u"
  service-accounts = toset([
    "nictaporuk-catgpt-sa",
  ])
...
resource "yandex_resourcemanager_folder_iam_member" "catgpt-roles" {
  ...
  folder_id = local.folder_id
  member    = "serviceAccount:${yandex_iam_service_account.service-accounts["nictaporuk-catgpt-sa"].id}"
  role      = each.key
}
...
resource "yandex_compute_instance" "catgpt-1" {
    ...
    service_account_id = yandex_iam_service_account.service-accounts["nictaporuk-catgpt-sa"].id
    resources {
      cores         = 2
      memory        = 1
      core_fraction = 5
